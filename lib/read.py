...
@timeit
def read_df(csv_path: str, config: Dict) -> pd.DataFrame:
...
@timeit
def pandas_read_csv(csv_path: str, config: Dict) -> pd.DataFrame:
    return pd.read_csv(csv_path, encoding="utf-8", low_memory=False, dtype=config["dtype"], parse_dates=config["parse_dates"])


@timeit
def preview_df(train_csv: str, config: Dict, nrows: int=3000):
    ...
    log("Memory per row: {:0.2f} Kib".format(mem_per_row / 1024))

    df_size = (num_rows * mem_per_row) / 1024 / 1024
    log("Approximate dataset size: {:0.2f} MiB".format(df_size))
